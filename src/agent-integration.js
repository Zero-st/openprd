...
const CANONICAL_SKILLS = [
  ...
  },
  {
    ...
    body: [
      ...
      '- 大界面改动缺少实现前 3 方向效果图评审或用户未确认方向时，不要进入大 UI 实现；UI 任务有参考图但缺少 visual-compare 输出时，不要宣称视觉实现完成；没有参考图时先判断新建界面还是修改既有界面，新建界面缺少 3 方向方案评审、修改既有界面缺少修改前后截图对比时，都不要宣称视觉自检完成；如果只做了普通截图或 Computer 实测截图而没有截图实测证据板，也不要宣称视觉收口完成；如果对比图仍有明显偏差或漂移，先返工而不是把差异留给用户发现。',

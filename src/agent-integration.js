...
const CANONICAL_SKILLS = [
  ...
  },
  {
    ...
    body: [
      ...
      '- 轻量 UI 可视优化证据：卡片宽度、间距、留白、对齐、颜色、圆角、字号、按钮或图标等小改仍属于用户可见变化。质量审查时至少确认存在修改前后视觉对比、局部焦点证据板或截图实测证据板；build、package、dev-check、单元测试或单张原始截图都不能替代视觉收口证据。',
      ...
      '- 大界面改动缺少实现前 3 方向效果图评审或用户未确认方向时，不要进入大 UI 实现；UI 任务有参考图但缺少 visual-compare 输出时，不要宣称视觉实现完成；没有参考图时先判断新建界面还是修改既有界面，新建界面缺少 3 方向方案评审、修改既有界面缺少修改前后截图对比时，都不要宣称视觉自检完成；轻量 UI 可视优化缺少视觉证据时，可以说代码或构建已处理，但不要说界面已经优化完成；如果只做了普通截图或 Computer 实测截图而没有截图实测证据板，也不要宣称视觉收口完成；如果对比图仍有明显偏差或漂移，先返工而不是把差异留给用户发现。',
...
const CANONICAL_COMMANDS = [
  ...
  },
  {
    ...
    body: [
      ...
      'When ordinary screenshots or Computer/Browser/Playwright checks are used as visual or runtime evidence, use `openprd visual-compare . --board <board.json>` with `mode=verification-board` to assemble screenshots, checked path, and checkpoints into one screenshot verification board. Raw screenshots are inputs, not final visual closeout evidence.',
      'Lightweight visible UI fixes such as card width, spacing, whitespace, alignment, colors, border radius, font size, buttons, or icons still need visual evidence before claiming the UI is done; builds, packages, dev-check, unit tests, and raw single screenshots are not substitutes.',
...
function renderCommandCatalog() {
  return [
    ...
    '- `openprd visual-compare . --board <board.json>`：当要审局部细节、整板局部映射或多对象验收时输出“局部焦点证据板”；当并行跑了多个优化方向时输出“并行实验证据板”；当普通截图、Computer/Browser/Playwright 实测截图要作为证据时输出“截图实测证据板”。',
    '- 轻量 UI 可视优化（卡片宽度、间距、留白、对齐、颜色、圆角、字号、按钮、图标等）也要留下视觉证据；build、package、`openprd dev-check`、单张原始截图不能替代 `visual-compare` 或证据板。',

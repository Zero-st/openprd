...
- 轻量 UI 可视优化：卡片宽度、间距、留白、对齐、颜色、圆角、字号、按钮或图标等小改仍属于用户可见变化。质量审查时至少确认存在修改前后视觉对比、局部焦点证据板或截图实测证据板；build、package、`dev-check`、单元测试或单张原始截图都不能替代视觉收口证据。
...
- 轻量 UI 可视优化缺少视觉证据时，可以说代码或构建已处理，但不要说“界面已经优化完成”。
